...
def estimate_intrinsics_with_pi3x(image: Image.Image, device: torch.device,pi3x_path ) -> np.ndarray:
    ...
    from ..Pi3.pi3.utils.geometry import recover_intrinsic_from_rays_d
...
def write_video(output_dir: Path, name: str, video_hwc: np.ndarray, fps: int, ) -> Path:
...
def load_sana_wm_model(model_path,node_cr_path,weight_dtype=torch.bfloat16):

    model_config = ModelVideoCamCtrlConfig(
    ...
    latent_size = config.model.image_size // 32
    kwargs = model_video_camctrl_init_config(config, latent_size=latent_size)
    if 'null_embed_path' not in kwargs or kwargs['null_embed_path'] is None:
        kwargs['null_embed_path'] = ''
    ...
    print(f"Loaded {config.model.model} ({sum(p.numel() for p in model.parameters()):,} params)")
    ...
    missing_unexpected = model.load_state_dict(state["state_dict"], strict=False,)

    print(missing_unexpected)
    
    del state
    model = model.eval().to(weight_dtype)
    model.config = config
    return model

def load_sana_wm_vae(vae_path, device,node_cr_path, vae_dtype=torch.bfloat16):

    vae_config = AEConfig(
        vae_type="LTX2VAE_diffusers",
        vae_pretrained=os.path.join(node_cr_path, "Sana/ltx2_vae/config.json"),
    ...
    # 检查并启用VAE的tiling功能（如果支持）
    if hasattr(vae, "enable_tiling"):
        vae.enable_tiling()
    # 检查并启用VAE的帧级编码/解码功能（如果支持）
    if hasattr(vae, "use_framewise_encoding"):
        vae.use_framewise_encoding = True
        vae.use_framewise_decoding = True
        # 设置帧采样相关的参数，使用配置中的值或默认值
        vae.tile_sample_stride_num_frames = getattr(vae_config, "tile_sample_stride_num_frames", 64)
        vae.tile_sample_min_num_frames = getattr(vae_config, "tile_sample_min_num_frames", 96)
    return vae


def load_sana_wm_text_encoder(text_encoder_path, node_sana_wm_path):
    tokenizer, text_encoder = get_tokenizer_and_text_encoder(name="gemma-2-2b-it", device=torch.device("cpu"),local_path=os.path.join(node_sana_wm_path, "Sana/gemma-2-2b-it"), model_path=text_encoder_path)      
    return tokenizer, text_encoder



def gemma_encode_prompts(text_encoder,tokenizer,streaming_prefetch_count,
         prompt: str, negative_prompt: str,device
        ...
        def encode(text: str, length: int) -> tuple[torch.Tensor, torch.Tensor]:
            tok = tokenizer([text], max_length=length, padding="max_length", truncation=True, return_tensors="pt"
            ).to(device)
            if streaming_prefetch_count is not None:
                with _model_ctx_(text_encoder, streaming_prefetch_count):
                    out =text_encoder(tok.input_ids, tok.attention_mask,)
                    cond=out[0]
            else:
                out =text_encoder(tok.input_ids, tok.attention_mask)
                cond=out[0]
            return cond , tok.attention_mask
      
        cond, cond_mask = encode(prompt, max_length_all)
        print(f"Prompt tokenized to {cond.shape[1]} tokens (max {max_length_all}).")
        ...
        #self.text_encoder.to("cpu") # TODO: offload
        return cond, cond_mask, neg[:, None], neg_mask


def prepare_camera_intrinsics(pi3x_path,image,num_frames,action,translation_speed,rotation_speed_deg,camera,device,save_intrinsics,save_path,intrinsics=""):

    c2w_full = _resolve_trajectory(action,translation_speed, rotation_speed_deg,camera)
    ...
    if snapped != num_frames:
        print(
            ...
            f"--num_frames={num_frames} snapped to {snapped} "
    ...
    if intrinsics:
        intr_src = load_intrinsics(intrinsics, num_frames)
    else:
        intr_one = estimate_intrinsics_with_pi3x(image, device,pi3x_path)
        intr_src = np.broadcast_to(intr_one, (num_frames, 4)).copy()
        if save_intrinsics:
            np.save(save_path, intr_src)
            print(f"Saved estimated intrinsics to {save_path} for future reuse.")
...
def infer_sana_wm(
    ...
    pipeline.vae = vae
    pipeline.streaming_prefetch_count = block_num

    params = GenerationParams(
        num_frames=cam_cond["num_frames"],
        fps=fps,
        step=step,
        cfg_scale=cfg_scale,
    ...
    out = pipeline.generate(cam_cond["cropped"], te_cond, cam_cond["c2w"], cam_cond["intrinsics_vec4"], params)
    video_hwc = out["video"]
    return torch.from_numpy((video_hwc.astype(np.float32)) / 255.0),out["latent"]


def load_refiner(
    model_path,  # 模型路径参数
    config_root,  # 配置文件根目录参数
    weight_dtype: torch.dtype = torch.bfloat16,
    device: torch.device | str = "cuda",

):
    """Load a LTX-2 refiner model."""

    refiner = DiffusersLTX2Refiner(
        refiner_root=config_root,
        model_path=model_path,
        dtype=weight_dtype,
        device=device,
    )
    refiner._load_diffusers_components()
    return refiner
...
def infer_refiner(
    ...
    print(f"[refiner] {len(sigmas) - 1}-step Euler, start_sigma={start_sigma:.4f}")
    ...
    #             return _streaming_model(
    #                 model,
    #                 layers_attr="transformer.transformer_blocks",
    #                 target_device=torch.device("cuda"),
    #                 prefetch_count=streaming_prefetch_count,
    #             )

    #         return model
    # with _model_ctx_(refiner, streaming_prefetch_count=block_num):
    refined = refiner.refine_latents(
        sana_latent,
        cond["prompt_embeds"].to(device),
        cond["prompt_attention_mask"].to(device),
        fps=float(fps),
        sink_size=1,
        seed=int(seed),
        progress=True,
    ...
    print(f"[refiner] decoding {refined.shape[2]} latent frames with diffusers LTX2 VAE")
    return refined
    # video = decode_with_sana_vae_(refined)
...
def decode_with_sana_vae_(vae, sana_latent: torch.Tensor,device,enable_tile,vae_dtype=torch.bfloat16,offload_vae=True) -> np.ndarray:
    print(f"[sana-vae] decoding {sana_latent.shape[2]} latent frames")
    if offload_vae:
        vae.to(device)
    if enable_tile:
        vae.enable_tiling()
    samples = sana_latent.to(device=device, dtype=vae_dtype)
    decoded = vae_decode("LTX2VAE_diffusers", vae, samples)
    if isinstance(decoded, list):
        decoded = torch.stack(decoded, dim=0)
    video = (
            (decoded / 2.0 + 0.5).clamp(0, 1).permute(0, 2, 3, 4, 1).to("cpu", dtype=torch.float32)[0]
        )

    #video = (torch.clamp(127.5 * decoded + 127.5, 0, 255).permute(0, 2, 3, 4, 1).to("cpu", dtype=torch.uint8).numpy()[0])
    if offload_vae:
        vae.to("cpu")
    del samples, decoded
    gc.collect()
...
class SanaWMPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.refiner_settings = None
        self.offload_vae = True
        self.streaming_prefetch_count = streaming_prefetch_count if streaming_prefetch_count > 0 else None
    ...
    def _build_vae(self) -> None:
        # 解析预训练VAE模型的路径，支持相对路径和绝对路径
        self.config.vae.vae_pretrained = os.path.join(self.model_path, "vae")
        #self.config.vae.vae_pretrained = resolve_hf_path(self.config.vae.vae_pretrained)
        # 根据配置获取VAE模型实例
        # 参数包括VAE类型、预训练路径、设备、数据类型和配置
        vae = get_vae(
    ...
    def _build_text_encoder(self) -> None:
        self.tokenizer, self.text_encoder = get_tokenizer_and_text_encoder(
            name=self.config.text_encoder.text_encoder_name, device=torch.device("cpu"),local_path=os.path.join(self.model_path, "gemma-2-2b-it")
        )
    ...
    def _build_model(self, model_path: str | Path) -> None:
        ...
        if self.streaming_prefetch_count is  None:
            model = model.to(self.device)
        self.logger.info(f"Loaded {self.config.model.model} ({sum(p.numel() for p in model.parameters()):,} params)")

        state = find_model(str(os.path.join(self.model_path, "dit/sana_wm_1600m_720p.safetensors")))
        if "generator" in state:
            state = state["generator"]
        if "state_dict" not in state:
            stripped = {(k[len("model.") :] if k.startswith("model.") else k): v for k, v in state.items()}
    ...
    def generate(
        ...
        sana_latent = self._sample_stage1(image, te_cond, camera, params, latent_T, latent_h, latent_w)

        if self.refiner_settings is not None:
            video = self._refine(sana_latent, te_cond, params, self.refiner_settings)
    ...
    def _encode_prompts(
        ...
        def encode(text: str, length: int) -> tuple[torch.Tensor, torch.Tensor]:
            ...
            model_context = _model_ctx_(self.text_encoder, self.streaming_prefetch_count) if self.streaming_prefetch_count is not None else self.text_encoder
            with model_context as  self.text_encoder:
                cond =self.text_encoder(tok.input_ids, tok.attention_mask)[0]
            return cond , tok.attention_mask
    ...
    def _sample_stage1(
        self,
        image: Image.Image,
        te_cond: tuple[torch.Tensor, torch.Tensor],
        ...
        cond, cond_mask, neg, neg_mask = te_cond["cond"].to(self.device, dtype=self.weight_dtype), te_cond["cond_mask"].to(self.device, dtype=self.weight_dtype), te_cond["neg"].to(self.device, dtype=self.weight_dtype), te_cond["neg_mask"].to(self.device, dtype=self.weight_dtype)
        torch.cuda.empty_cache()
    ...
    def _dispatch_solver(
        ...
        def _model_ctx(model,
            ...
            return model
        model_context = _model_ctx(self.model, self.streaming_prefetch_count) if self.streaming_prefetch_count is not None else self.model
    ...
    def _decode_with_sana_vae(self, sana_latent: torch.Tensor) -> np.ndarray:
        ...
        decoded = vae_decode("LTX2VAE_diffusers", self.vae, samples)
        if isinstance(decoded, list):
            decoded = torch.stack(decoded, dim=0)
        video = (
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Number of frames to prefetch for streaming.",
    )

    return p


def _resolve_trajectory(action,translation_speed: float, rotation_speed_deg: float,camera=None) -> np.ndarray:
