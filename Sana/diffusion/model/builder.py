...
# from ..data.datasets.video.sana_video_data import SanaZipDataset
# from .dc_ae.efficientvit.ae_model_zoo import DCAE_HF, DCAEWithTemporal_HF
# from .qwen.qwen_vl import QwenVLEmbedder
from .utils import set_fp32_attention, set_grad_checkpoint
# from .wan2_2.vae import Wan2_2_VAE
# from .wan.clip import CLIPModel
...
class Registry:
    ...
    def _do_register(self, name, module, force=False):
        if name in self._module_dict and not force:
            raise KeyError(f'{name} already registered')
        self._module_dict[name] = module

    def build(self, cfg, default_args=None):
        # 根据 dict 中的 type 字段实例化
        cfg = cfg.copy()
        if default_args is not None:
            cfg.update(default_args)
        t = cfg.pop('type')
        if t not in self._module_dict:
            raise KeyError(f'{t} not found in {self._name} registry')
...
def get_tokenizer_and_text_encoder(name="T5", device="cuda",local_path=None,model_path=None, ):
    text_encoder_dict = {
    ...
    if "T5" in name:
        tokenizer = T5Tokenizer.from_pretrained(text_encoder_dict[name] if  local_path is None else local_path)
        text_encoder = T5EncoderModel.from_pretrained(text_encoder_dict[name] if  local_path is None else local_path, torch_dtype=torch.float16).to(device)
    elif "gemma" in name:
        tokenizer = AutoTokenizer.from_pretrained(text_encoder_dict[name] if  local_path is None else local_path)
        tokenizer.padding_side = "right"
        config_=Gemma2Config.from_pretrained(local_path)
        with init_empty_weights():
            text_encoder=Gemma2Model(config_)
        sd=safe_load_file(model_path)
        sd={key.replace("model.", ""): value for key, value in sd.items()}
        X=text_encoder.load_state_dict(sd, strict=False,assign=True)
        print(X)
        del sd
        text_encoder=text_encoder.eval().to(torch.float16)
        # text_encoder = (
        #     AutoModelForCausalLM.from_pretrained(text_encoder_dict[name] if  local_path is None else local_path, torch_dtype=torch.bfloat16)
        #     .get_decoder() 
        #     .to(device)
        # )
    elif "Qwen" in name:
        text_handler = QwenVLEmbedder(model_id=text_encoder_dict[name] if  local_path is None else local_path, device=device)
...
def get_vae(name, model_path, device="cuda", dtype=None, config=None):
    ...
    #     return vae
        ...
        vae=AutoencoderKLLTX2Video.from_single_file(model_path,config=config.vae_pretrained,  torch_dtype=dtype)
        # vae = (
